...
# Source commit: 1e8ae5d00162f3b4b92309ccd3ccd76f7dbab347
...
def backfill_spot(days: int, rows: dict[str, dict], cboe_hist: dict[str, dict[str, float]] | None=None, cboe_failed: set[str] | None=None, provisional_rows: set[str] | None=None) -> int:
...
def backfill_spot_cboe(rows: dict[str, dict], today: str | None=None, hist: dict[str, dict[str, float]] | None=None, failed: set[str] | None=None, provisional_rows: set[str] | None=None) -> dict[str, int]:
    if today is None:
    ...
    for d_str, row in rows.items():
        ...
        if not failed and d_str < today and (hist['vix'].get(d_str) is not None):
            if str(row.get('basis', '') or '').strip() != 'SETTLE':
                row['basis'] = 'SETTLE'
                settle_stamped += 1
    print(f'  CBOE: {agreed} cell(s) agreed, {filled} blank(s) filled, {corrected} CORRECTED, {settle_stamped} row(s) stamped SETTLE, {settle_withheld_provisional} row(s) NOT stamped (hold a provisional yfinance cell from this run)')
    if corrections:
...
def main(argv=None):
    ...
    if not args.m1m2_only:
        ...
        touched = backfill_spot(args.spot_days, rows, cboe_hist=cboe_hist, cboe_failed=cboe_failed, provisional_rows=provisional_rows)
        print(f'  touched {touched} rows')
        print('\n[1c] Writing spot columns from CBOE (authoritative)...')
        backfill_spot_cboe(rows, hist=cboe_hist, failed=cboe_failed, provisional_rows=provisional_rows)

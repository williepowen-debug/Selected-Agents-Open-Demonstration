...
# Source commit: 7b746e269163d84d0bec02cf1c3e58839e17552c
...
def backfill_spot(days: int, rows: dict[str, dict], cboe_hist: dict[str, dict[str, float]] | None=None, cboe_failed: set[str] | None=None) -> int:
    ...
    if cboe_hist is not None or failed:
        print(f'  yfinance (provisional): {provisional} cell(s) written where CBOE publishes nothing, {deferred_cboe} deferred to CBOE, {withheld_failed} WITHHELD (CBOE series failed this run)')
    return touched
    ...
    for d_str, row in rows.items():
        ...
        if not failed and d_str < today and (hist['vix'].get(d_str) is not None):
            if str(row.get('basis', '') or '').strip() != 'SETTLE':
                row['basis'] = 'SETTLE'
                settle_stamped += 1
    print(f'  CBOE: {agreed} cell(s) agreed, {filled} blank(s) filled, {corrected} CORRECTED, {settle_stamped} row(s) stamped SETTLE')
    if corrections:
        print(f'  🔴 {len(corrections)} VALUE CORRECTION(S) — ledger was wrong, CBOE wins:')
        for line in corrections:
            print(line)
    return {'filled': filled, 'corrected': len(corrections), 'agreed': agreed, 'settle_stamped': settle_stamped}
